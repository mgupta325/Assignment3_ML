...
plt.title('visualization of data in 2D (ica)-> digits dataset')
plt.show()
...
plt.ylabel('kurtosis')
plt.title('kurtosis analysis of digits dataset')

plt.subplot(2,1,2)
plt.plot(x1,r1)
plt.xlabel('number of components')
plt.ylabel('error')
plt.title('reconstruction error of digits dataset(ica)')
plt.subplots_adjust(hspace=0.54)
plt.show()

### kmeans clustering on 15 input features
fca = FastICA(n_components=15, random_state=77)
fca.fit(X)
X12 = fca.transform(X)
